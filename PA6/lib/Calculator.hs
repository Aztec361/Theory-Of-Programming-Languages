...
simplifyZero (Op Plus (Lit 0) e) = simplifyZero e
...
peephole :: (Expr -> Expr) -> Expr -> Expr
peephole f (Op op e1 e2) = f (Op op (peephole f e1) (peephole f e2))
peephole f (Lit n) = f (Lit n)
...
step (IOp op) (x:y:s) = Just ((interpBinOp op) x y : s)
step _ _ = Nothing
...
compile (Op op e1 e2) = compile e2 ++ compile e1 ++ [IOp op]
