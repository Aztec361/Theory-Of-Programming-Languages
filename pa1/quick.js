...
const fold_left = function (f, base, ls) {
  ...
  const newBase = f(base, ls.first());
    const newLs = ls.slice(1); // Remove the first element from the list
  return /** <FILL-IN> **/  fold_left(f, newBase, newLs); /** </FILL-IN> **/
};
...
const map = (g, ls) => /** <FILL-IN> **/ {return fold_left((acc, x) => acc.push(g(x)), List([]), ls);} /** </FILL-IN> **/
...
const filter = (g, ls) => /** <FILL-IN> **/  {return fold_left((acc, x) => (g(x) ? acc.push(x) : acc), List([]), ls);} /** </FILL-IN> **/
...
{const leftPartition = filter(g, ls);
const rightPartition = filter((x) => !g(x), ls);
return List([leftPartition, rightPartition]);} /** </FILL-IN> **/
...
const quicksort = function(ls) {
  ...
  /** <FILL-IN> **/
 // Pick the pivot (first element)
 const pivot = ls.size > 0 ? ls.first() : null;

 // Partition the list into two partitions
 const leftPartition = filter((x) => x <= pivot, ls.slice(1));
 const rightPartition = filter((x) => x > pivot, ls.slice(1));

 // Recursively sort the partitions
 const sortedLeft = quicksort(leftPartition);
 const sortedRight = quicksort(rightPartition);

 // Combine the sorted partitions and put the pivot back in
 return sortedLeft.concat(List([pivot])).concat(sortedRight);
  /** </FILL-IN> **/
};
